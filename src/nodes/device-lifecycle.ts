...
export const devicePrepareInputSchema = z.strictObject({
  target: z
    .literal('home')
    .describe('准备目标，首期仅支持 home（返回 Android 主屏）。'),
});
...
/**
 * device.prepare：准备当前绑定设备，首期唯一目标是返回 Android 主屏。
 * 只提供原生导航基线；不处理解锁、网络重置、恢复出厂或业务初始状态。
 */
...
/**
 * device.recover：结束恢复当前绑定设备，唯一行为是返回 Android 主屏。
 * 保留当前系统设置与使用方业务状态；恢复成功仅表示完成 Home 基线恢复。
 */
...
function requireProjectAgent(
  ...
  if (!context?.agent) {
    throw new Error(
      `${nodeName} 需要 android 项目 setup 提供的 AndroidAgent；请确认用例在 android 执行项目中运行。`,
    );
  }
