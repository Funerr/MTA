...
本路线图对应当前已规划的 9 个 Change（含 1 个新增的双平台接入）。提案、规格、设计和任务清单均已生成；这表示规划就绪，不表示工程或 Experience 能力已经实现。实施按下表推进，前序验证完成后再进入依赖它的 Change。
...
| 3 | [add-experience-promotion](changes/add-experience-promotion/proposal.md) | 🔶 实施完成（9/9 任务、130 项测试通过），待 verify/archive | 原生轨迹取数契约与 candidate 生成 | 2 |
| 4 | [add-harmony-platform](changes/add-harmony-platform/proposal.md) | 📋 规划就绪，待实施 | HarmonyOS 会话、双执行项目、项目本地原生节点 | 1 |
| 5 | [add-visual-matcher](changes/add-visual-matcher/proposal.md) | 📋 规划就绪 | 本地页面/目标/状态匹配 | 2、3 |
...
- 动作 Experience MVP 主链为 1 → 2 → 3 → 5 → 6 → 7；8 在 MVP 验收后接入原生用例语法；9 是独立研究分支，不要求先完成 8，也不阻塞动作 MVP。
- 4（双平台接入）只依赖 1，与 Experience 链（2、3、5–9）互不前置，可在 3 归档后随时并行实施；Experience 链建立在平台无关的 Agent/资产层之上，双平台化不改变其前提。
- 顺序 5–9 的编号自本版起因插入 4 而整体后移一位，各提案正文中的"实施前置"仍按 Change 名称互相引用，不受编号影响。
...
未归档的规划目录均包含 `proposal.md`、`design.md`、`tasks.md` 和 `specs/<capability>/spec.md`（add-harmony-platform 有三份规格：新增 `midscene-harmony-project`，修改 `midscene-android-project` 与 `device-lifecycle-nodes`）。
