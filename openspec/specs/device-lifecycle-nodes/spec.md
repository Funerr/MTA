# device-lifecycle-nodes Specification
...
### Requirement: 生命周期失败可见

两个设备 Node SHALL 传播设备操作失败，由原生运行器处理超时和失败状态；MUST 不吞掉异常、不私自重试、不把执行失败报告为恢复成功。

#### Scenario: 恢复期间设备断连

- **WHEN** `device.recover` 执行时设备连接已断开
- **THEN** 结果记录恢复失败，命令以非零状态结束；如已有用例失败，两种失败信息都可追踪

#### Scenario: 准备失败阻止测试步骤

- **WHEN** `device.prepare` 发生设备操作错误
- **THEN** 用例主体步骤不执行，原生生命周期仍处理 `afterEach`，最终结果保持失败
